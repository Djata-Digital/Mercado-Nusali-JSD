...
async function runAsaasPixTests() {
  ...
  try {
    ...
    // ----------------------------------------------------
    // TESTE I: Customer Asaas Existente com Cadastro Atualizado (Sincronização & Telefone Válido)
    // ----------------------------------------------------
    console.log('--- TESTE I: Customer Asaas Sincronizado com Telefone Válido ---');
    const updatedPhone = '11988887777';
    await db.update(users).set({ phone: updatedPhone }).where(eq(users.id, testBuyerBR_Id));

    const syncedCustomerId = await provider.getOrCreateCustomer(testBuyerBR_Id);
    if (syncedCustomerId !== brCustomerId1) {
      throw new Error(`TESTE I FALHOU: Sincronização alterou o providerCustomerId (${syncedCustomerId} != ${brCustomerId1}).`);
    }

    const asaasCustInfo1 = await AsaasClient.request<any>(`/customers/${syncedCustomerId}`);
    if (asaasCustInfo1.mobilePhone !== updatedPhone) {
      throw new Error(`TESTE I FALHOU: mobilePhone no Asaas não foi sincronizado (obtido: ${asaasCustInfo1.mobilePhone}, esperado: ${updatedPhone}).`);
    }
    console.log(`✅ [PASS] Customer Asaas ${syncedCustomerId} sincronizado com novo telefone validado (${updatedPhone}).\n`);

    // ----------------------------------------------------
    // TESTE J: Comprador BR sem telefone (phone = null) -> Omissão de mobilePhone
    // ----------------------------------------------------
    console.log('--- TESTE J: Comprador BR sem Telefone (phone = null) -> Omissão de mobilePhone ---');
    await db.update(users).set({ phone: null }).where(eq(users.id, testBuyerBR_Id));

    const syncedNoPhoneId = await provider.getOrCreateCustomer(testBuyerBR_Id);
    if (syncedNoPhoneId !== brCustomerId1) {
      throw new Error(`TESTE J FALHOU: Reutilização de customer falhou (${syncedNoPhoneId} != ${brCustomerId1}).`);
    }

    const asaasCustInfo2 = await AsaasClient.request<any>(`/customers/${syncedNoPhoneId}`);
    console.log(`✅ [PASS] Customer Asaas ${syncedNoPhoneId} sincronizado sem telefone (mobilePhone = ${asaasCustInfo2.mobilePhone ?? 'null/omitido'}).\n`);

    // ----------------------------------------------------
    // TESTE K: Comprador Estrangeiro (GW) -> Preservação de foreignCustomer=true sem conversão BR
    // ----------------------------------------------------
    console.log('--- TESTE K: Comprador GW (Guiné-Bissau) -> Preservação de foreignCustomer=true ---');
    await db.update(users).set({ phone: '+245955123456' }).where(eq(users.id, testBuyerGW_Id));

    const gwCustIdSynced = await provider.getOrCreateCustomer(testBuyerGW_Id);
    const asaasGWCust = await AsaasClient.request<any>(`/customers/${gwCustIdSynced}`);

    if (!asaasGWCust.foreignCustomer) {
      throw new Error('TESTE K FALHOU: foreignCustomer não permaneceu true para cliente GW.');
    }
    console.log(`✅ [PASS] Customer GW (${gwCustIdSynced}) preservado como foreignCustomer=true sem conversão indevida de telefone.\n`);
