  ...
  try {
    ...
    const data = sellerRows.map((s: any) => {
      ...
      return {
        ...
        tradingName: s.tradingName || s.companyName || user?.fullName || 'Loja Nusali',
        email: user?.email || '',
        phone: s.phone || user?.phone || '',
        countryCode: s.countryCode || user?.countryCode || 'GW',
        status: s.status || 'pending',
        taxId: s.taxId || 'N/A',
        commissionRate: s.commissionRate || '8.00',
        rating: s.rating || '5.00',
    ...
    sellerUsers.forEach((u: any) => {
      ...
      if (!alreadyIncluded) {
        data.push({
          ...
          tradingName: u.fullName || 'Loja em Configuração',
          email: u.email || '',
          phone: u.phone || '',
          countryCode: u.countryCode || 'GW',
          status: 'pending',
          taxId: 'Pendente',
          commissionRate: '8.00',
          rating: '5.00',
...
adminRouter.get('/logistics/shipments', requireLogisticsStaff, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (fulfillmentMode && fulfillmentMode !== 'todos') {
      filtered = filtered.filter(s => s.fulfillmentMode === fulfillmentMode);
    }

    if (countryCode && countryCode !== 'todos') {
      filtered = filtered.filter(s => s.originCountry === countryCode || s.destinationCountry === countryCode);
    }
