...
export const TrackingView: React.FC = () => {
  ...
  const realTrackingCode = activeOrder.trackingCode || activeOrder.shipment?.trackingNumber || null;

  const copyTrackingCode = () => {
    if (realTrackingCode) {
      navigator.clipboard.writeText(realTrackingCode);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn">
      ...
      <button
        ...
        className="flex items-center gap-2 text-xs font-bold text-gray-600 hover:text-emerald-700 mb-6 transition cursor-pointer"
      >
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-gray-200 mt-1">
              Transportadora: <span className="font-bold text-yellow-300">{activeOrder.carrierName || activeOrder.carrier || 'Transportadora não definida'}</span>
            </p>
          </div>
          ...
          <div className="bg-white/10 p-4 rounded-xl border border-white/20 backdrop-blur-xs text-xs font-mono w-full md:w-auto">
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="font-bold text-yellow-300 text-sm tracking-wider">{realTrackingCode || 'Envio não gerado'}</span>
              {realTrackingCode && (
                <button
                  onClick={copyTrackingCode}
                  className="p-1.5 bg-white/20 hover:bg-white/30 rounded text-white transition cursor-pointer"
                  title="Copiar Código"
                >
