...
export const RegionalOperationsView: React.FC<RegionalOperationsViewProps> = ({ showToast }) => {
  ...
  if (mockRegionsList.length === 0 || !regionData) {
    return (
      <div className="space-y-6">
        <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-xs">
          ...
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <MapPin className="w-6 h-6 text-emerald-600" /> Operações Regionais
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Supervisão direta das regiões, acompanhamento dos vendedores e rotas operacionais.
          </p>
        </div>

        <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-12 text-center text-gray-400 space-y-2">
          <MapPin className="w-10 h-10 mx-auto text-gray-300 stroke-1" />
          <p className="font-bold text-sm text-gray-600">Nenhuma região cadastrada</p>
          <p className="text-xs text-gray-400 max-w-sm mx-auto">
            Não existem regiões ou setores operacionais cadastrados no sistema. Adicione regiões na tela de Gestão de Regiões para visualizar o painel regional.
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 space-y-4">
        <h3 className="font-extrabold text-sm text-gray-900">
          Cidades, Bairros e Setores de {regionData.name} ({regionData.cities?.length || 0})
        </h3>
