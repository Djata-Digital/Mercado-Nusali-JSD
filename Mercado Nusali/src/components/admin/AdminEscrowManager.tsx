  ...
  const handleManualRelease = async (id: string) => {
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || `Custódia #${id} liberada manualmente. Saldo transferido ao vendedor.`);
        fetchEscrow();
      } else {
        showToast(res.message || `Falha ao liberar custódia #${id}.`);
      }
    } catch (err: any) {
      showToast(err?.message || `Erro ao tentar liberar custódia #${id}.`);
    }
  };
  ...
  const handleBlock = async (id: string) => {
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || `Falha ao bloquear custódia #${id}.`);
      }
    } catch (err: any) {
      showToast(err?.message || `Erro ao tentar bloquear custódia #${id}.`);
    }
  };
