...
export const inMemoryStore = {
  ...
  regions: [
    { id: 'REG-GW-001', name: 'Região de Bissau', countryCode: 'GW', supervisorName: 'Malam Bacai', supervisorEmail: 'malam@nusali.gw', deliveryCoverageDays: '24h', freightBaseRate: '1.000 XOF' },
    { id: 'REG-AO-001', name: 'Província de Luanda', countryCode: 'AO', supervisorName: 'Ana Paula Santos', supervisorEmail: 'ana.luanda@nusali.ao', deliveryCoverageDays: '24-48h', freightBaseRate: '2.500 AOA' },
  ],
  warehouses: [
    { id: 'wh_001', code: 'HUB-BIS-01', name: 'HUB Central Bissau', countryCode: 'GW', city: 'Bissau', address: 'Avenida Amílcar Cabral, Bissau', managerName: 'Domingos Vaz', staffCount: 18 },
    { id: 'wh_002', code: 'HUB-LUA-01', name: 'HUB Logístico Luanda', countryCode: 'AO', city: 'Luanda', address: 'Zona Econômica Especial Viana, Luanda', managerName: 'Mariana Costa', staffCount: 35 },
  ],
  users: [
...
apiRouter.post('/products', async (req: Request, res: Response) => {
  ...
  const newProduct = {
    ...
    image: image || 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?auto=format&fit=crop&q=80&w=800',
    ...
    shipping: shipping || { freeShipping: true, arrivesTomorrow: true, shippingPrice: 0, fullFulfilled: true },
    seller: seller || {
      id: 'sel_current',
  ...
  try {
    ...
    if (isConnected) {
      ...
      if (db) {
        ...
        return res.json({
          success: true,
          message: 'Produto cadastrado no banco de dados PostgreSQL com sucesso!',
          data: newProduct,
        });
      }
    }
  } catch (err: any) {
    console.warn('PostgreSQL unavailable, storing product in memory.');
  }

  inMemoryStore.products.unshift(newProduct);
  return res.json({
    success: true,
    message: 'Produto cadastrado em memória com sucesso!',
    data: newProduct,
  });
});
