...
pixRouter.post('/confirm/:txid', (req: Request, res: Response) => {
  ...
  // Update order in buyerDataStore if linked
  if (transaction.orderId) {
    const order = buyerDataStore.orders.find(
      (o) => o.id === transaction.orderId || o.orderNumber === transaction.orderId
    );
    if (order) {
      order.status = 'paid';
      order.escrowStatus = 'held';
      order.paymentMethod = 'PIX Brasil (Banco Central)';
      order.paymentDetails = {
        method: 'pix',
        currency: 'BRL',
        total: transaction.amountBrl,
        pixCode: transaction.brCode,
        transactionRef: transaction.endToEndId,
      } as any;
    }
  }
