...
export const SellerOrdersManager: React.FC<SellerOrdersManagerProps> = ({
  ...
  const getOrderLabelData = (ord: SellerOrderData): ShippingLabelData => ({
    trackingNumber: `NX-${ord.buyerCountry || 'GW'}-${ord.orderNumber.replace(/\D/g, '') || '849204'}`,
    orderNumber: ord.orderNumber,
    serviceType: 'NUSALI EXPRESS FULL CPLP',
    routeCode: `GW-BIS > ${ord.buyerCountry || 'GW'}-${(ord.deliveryCity || 'CAP').substring(0, 3).toUpperCase()} > HUB-01`,
    destinationHub: `HUB-${(ord.deliveryCity || 'CENTRAL').toUpperCase()}`,
    sender: {
      name: 'Vendedor Oficial Nusali',
      storeName: ord.storeName || 'Loja Principal',
      address: 'Av. dos Combatentes da Liberdade, HUB Bandim',
      city: 'Bissau',
      country: 'GW',
      phone: '+245 955000111',
    },
    recipient: {
      name: ord.buyerName,
      address: `Avenida Principal, ${ord.deliveryCity}`,
      city: ord.deliveryCity,
      country: ord.buyerCountry,
      phone: ord.buyerPhone,
      postalCode: ord.buyerCountry === 'PT' ? '1000-001' : ord.buyerCountry === 'BR' ? '01310-100' : 'CP-1000',
    },
    packageInfo: {
      itemsDescription: `${ord.productTitle}${
      ...
      sku: ord.selectedVariantSku || ord.productSku,
      quantity: ord.quantity,
      weightKg: (ord.quantity * 0.48).toFixed(2),
      dimensions: '22 × 16 × 8 cm',
      declaredValue: formatCurrency(ord.totalAmount, selectedCurrency),
    },
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-xs">
                ...
                <div className="text-right shrink-0 space-y-1">
                  ...
                  <div className="pt-2 flex items-center justify-end gap-2">
                    <button
                      onClick={() => setActiveLabelData(getOrderLabelData(ord))}
                    ...
                    <button
                      ...
                      className="p-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition flex items-center gap-1 shadow-xs cursor-pointer"
                    >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar text-xs">
            ...
            <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
              <button
                type="button"
                onClick={() => setActiveLabelData(getOrderLabelData(selectedOrder))}
