...
export const SellerFinancialManager: React.FC<SellerFinancialManagerProps> = ({
  ...
  const handleRequestPayout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.error?.message || err?.message || 'Erro ao processar saque.');
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gradient-to-br from-emerald-900 to-teal-950 text-white p-6 rounded-2xl shadow-md space-y-2">
          ...
          <span className="text-[11px] text-emerald-300 font-semibold block">
            Prazo e tarifas dependem do provedor, país e método selecionado.
          </span>
        </div>
        ...
        <div className="divide-y divide-gray-100 text-xs">
          {ledgerTransactions.length === 0 ? (
            <div className="p-8 text-center text-gray-400">
              ...
              <p className="font-bold text-sm text-gray-600">Nenhum lançamento financeiro registrado no extrato.</p>
            </div>
          ) : (
