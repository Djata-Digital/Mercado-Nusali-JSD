...
export const SellerOverview: React.FC<SellerOverviewProps> = ({
  ...
  const grossRevenue = analyticsData?.grossRevenue || overviewData?.metrics?.grossRevenue || 14850000;
  const netRevenue = analyticsData?.netRevenue || overviewData?.metrics?.netRevenue || 14107500;
  const availableBalance = overviewData?.balances?.available || 1250000;
  const escrowBalance = overviewData?.balances?.retained || 450000;
  const pendingRelease = overviewData?.balances?.future || 320000;
  ...
  const countryOrders = [
    { country: '🇬🇼 Guiné-Bissau', pedidos: 62, valor: formatCurrency(8200000, selectedCurrency) },
    { country: '🇵🇹 Portugal', pedidos: 22, valor: formatCurrency(3500000, selectedCurrency) },
    { country: '🇧🇷 Brasil', pedidos: 12, valor: formatCurrency(1900000, selectedCurrency) },
    { country: '🇦🇴 Angola', pedidos: 6, valor: formatCurrency(1250000, selectedCurrency) },
  ];
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-2xs">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 text-center">
          <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
            <span className="text-lg font-black text-gray-900 block">{overviewData?.metrics?.pendingOrders || 1}</span>
            <span className="text-[11px] font-bold text-amber-600 block">Pendentes</span>
          </div>
          <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
            <span className="text-lg font-black text-gray-900 block">{overviewData?.metrics?.totalOrders || 3}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-2xs">
          ...
          <span className="text-xl font-black text-amber-500 flex items-center gap-1 mt-1">
            {overviewData?.metrics?.averageRating || 4.9} <Star className="w-4 h-4 fill-amber-500" />
          </span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-2xs">
          ...
          <span className="text-xl font-black text-gray-900 mt-1 block">
            {overviewData?.metrics?.totalProducts || 3}
          </span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-2xs">
          ...
          <span className="text-xl font-black text-emerald-700 mt-1 block">
            {analyticsData?.conversionRate || '3.8%'}
          </span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-2xs">
          ...
          <span className="text-xl font-black text-gray-900 mt-1 block">
            {analyticsData?.viewsCount?.toLocaleString() || '14.200'}
          </span>
        </div>
        ...
        >
          ...
          <span className="text-xl font-black text-amber-600 mt-1 block">
            {overviewData?.metrics?.pendingQuestions || 1}
          </span>
        </div>
