...
export const SellerReturnsManager: React.FC<SellerReturnsManagerProps> = ({ showToast }) => {
  ...
  const getReturnLabelData = (item: ReturnItem): ShippingLabelData => ({
    trackingNumber: `RET-TRK-${item.id.replace(/\D/g, '') || '9821'}`,
    orderNumber: `PEDIDO ${item.orderId} / DEVOLUÇÃO ${item.id}`,
    serviceType: 'LOGÍSTICA REVERSA NUSALI CPLP',
    routeCode: 'DEV-CLI > HUB-SELLER > 001',
    destinationHub: 'HUB-GW-01-CENTRAL',
    sender: {
      name: item.buyerName,
    ...
    recipient: {
      name: 'Vendedor Mercado Nusali - Armazém de Retorno',
    ...
    packageInfo: {
      itemsDescription: `Devolução: ${item.productTitle}`,
      sku: `DEV-${item.id}`,
      quantity: 1,
      weightKg: '0.60',
      dimensions: '20 × 15 × 8 cm',
      declaredValue: `${item.currency} ${item.amount.toLocaleString()}`,
    },
    issuedAt: item.requestDate,
  });
