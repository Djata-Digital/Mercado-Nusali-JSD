...
export const SellerProductWizard: React.FC<SellerProductWizardProps> = ({
  ...
  const [category, setCategory] = useState(initialProduct?.category || mockCategories[0].name);
  ...
  const [variantsMatrix, setVariantsMatrix] = useState<ProductVariant[]>(() => {
    ...
    // Generate default matrix from colors & sizes
    return [
      { id: 'var-1', color: 'Preto', size: 'P', stock: 5 },
      { id: 'var-2', color: 'Preto', size: 'M', stock: 10 },
      { id: 'var-3', color: 'Preto', size: 'G', stock: 5 },
      { id: 'var-4', color: 'Branco', size: 'P', stock: 3 },
      { id: 'var-5', color: 'Branco', size: 'M', stock: 8 },
      { id: 'var-6', color: 'Branco', size: 'G', stock: 4 },
    ];
  });

  // Product Kits (2x, 5x, 10x)
  const [productKits, setProductKits] = useState<ProductKit[]>(() => {
    ...
    return [
      { id: 'kit-2', quantity: 2, title: 'Kit com 2 Unidades', discountPercentage: 10, badge: 'Economize 10%' },
      { id: 'kit-5', quantity: 5, title: 'Kit com 5 Unidades', discountPercentage: 18, badge: 'Mais Vendido' },
      { id: 'kit-10', quantity: 10, title: 'Kit com 10 Unidades (Atacado)', discountPercentage: 25, badge: 'Preço de Atacado' },
    ];
  });
  ...
  const [price, setPrice] = useState(initialProduct?.price ? String(initialProduct.price) : '25000');
  const [originalPrice, setOriginalPrice] = useState(
    initialProduct?.originalPrice ? String(initialProduct.originalPrice) : '30000'
  );
  const [stock, setStock] = useState(
    initialProduct?.stock !== undefined ? String(initialProduct.stock) : '35'
  );
  const [warehouseHub, setWarehouseHub] = useState<'HUB-GW-01' | 'HUB-PT-02' | 'HUB-BR-03'>(
    (initialProduct?.specs?.Armazém as any) || 'HUB-GW-01'
  );
  const [warrantyMonths, setWarrantyMonths] = useState(
    initialProduct?.specs?.Garantia ? initialProduct.specs.Garantia.replace(/\D/g, '') || '12' : '12'
  );

  // Weight & Dimensions
  const [weightKg, setWeightKg] = useState(
    initialProduct?.weightKg
  ...
  const [lengthCm, setLengthCm] = useState(
    initialProduct?.dimensionsCm?.length
      ? String(initialProduct.dimensionsCm.length)
      : '20'
  );
  const [widthCm, setWidthCm] = useState(
    initialProduct?.dimensionsCm?.width
      ? String(initialProduct.dimensionsCm.width)
      : '15'
  );
  const [heightCm, setHeightCm] = useState(
    initialProduct?.dimensionsCm?.height
  ...
  useEffect(() => {
    if (initialProduct) {
      ...
      setCategory(initialProduct.category || mockCategories[0].name);
      ...
      setStock(initialProduct.stock !== undefined ? String(initialProduct.stock) : '10');
      setWarehouseHub((initialProduct.specs?.Armazém as any) || 'HUB-GW-01');
    ...
    if (currentColors.length === 0 && currentSizes.length === 0) {
      ...
      currentColors.forEach((c, cIdx) => {
        ...
        newMatrix.push({
          ...
          stock: existing?.stock ?? Math.max(5, Math.floor((parseInt(stock) || 10) / currentColors.length)),
          price: existing?.price ?? (basePriceNum > 0 ? basePriceNum : undefined),
          originalPrice: existing?.originalPrice ?? baseOrigPriceNum,
      ...
      currentSizes.forEach((s, sIdx) => {
        ...
        newMatrix.push({
          id: `var-s-${sIdx}`,
          size: s,
          stock: existing?.stock ?? Math.max(5, Math.floor((parseInt(stock) || 10) / currentSizes.length)),
          price: existing?.price ?? (basePriceNum > 0 ? basePriceNum : undefined),
          originalPrice: existing?.originalPrice ?? baseOrigPriceNum,
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          if (event.target?.result) {
            ...
            setGallery((prev) => [...prev, resultStr]);
          }
        };
    ...
    showToast(`${files.length} foto(s) carregada(s) com sucesso!`);
  ...
  const handleVideoFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setShortVideoUrl(event.target.result as string);
        showToast('Vídeo curto carregado com sucesso!');
      }
    };
    reader.readAsDataURL(file);
  ...
  const handleLoadSampleGallery = () => {
    ...
    setGallery(sampleSmartphones);
    showToast('Galeria de 4 fotos HD carregada!');
  };

  const handleLoadSampleVideo = () => {
    setShortVideoUrl('https://assets.mixkit.co/videos/preview/mixkit-hands-holding-a-smartphone-with-green-screen-42352-large.mp4');
    setShortVideoTitle('Vídeo Unboxing e Demonstração 360°');
    setShortVideoDuration('0:25');
    showToast('Vídeo curto demonstrativo de exemplo carregado!');
  };
    ...
    try {
      const res = await fetch('/api/gemini/seller-description', {
        ...
        body: JSON.stringify({
          title,
          category,
          brand: brand || 'Marca Oficial',
          specs: { Modelo: model, Condição: condition, Estoque: stock },
        }),
      });
      ...
      setDescription(
        `Anúncio Oficial: ${title}\n\nProduto de alta qualidade com garantia de fábrica. Embalagem original lacrada e envio expresso seguro via Nusali Logística Global.`
      );
      showToast('Descrição profissional gerada com sucesso!');
    } finally {
  ...
  const handlePublish = (e: React.FormEvent) => {
    ...
    const categoryObj = mockCategories.find((c) => c.name === category) || mockCategories[0];

    const parsedWeight = parseFloat(weightKg) || 0.5;
    const parsedLength = parseFloat(lengthCm) || 20;
    const parsedWidth = parseFloat(widthCm) || 15;
    const parsedHeight = parseFloat(heightCm) || 10;
    const dimensionsObj = { length: parsedLength, width: parsedWidth, height: parsedHeight };
    const formattedDimensionsStr = `${parsedLength} × ${parsedWidth} × ${parsedHeight} cm`;

    const mainCoverImage = gallery[0] || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500';
    ...
    if (isEditing && initialProduct && onUpdateProduct) {
      const updatedProduct: Product = {
        ...
        videos: shortVideoUrl
          ? [
              {
                url: shortVideoUrl,
                title: shortVideoTitle || 'Vídeo de Demonstração',
                duration: shortVideoDuration || '0:25',
                thumbnail: mainCoverImage,
              },
            ]
          : initialProduct.videos || [],
        videoUrl: shortVideoUrl || undefined,
        ...
        condition,
        brand: brand || initialProduct.brand || 'Marca Própria',
        model: model || initialProduct.model || 'Padrão',
        stock: parseInt(stock) || 10,
        ...
        shipping: {
          ...
          originCountry,
          targetCountries: effectiveTargetCountries,
          originCity: originCountry === 'GW' ? 'Bissau' : originCountry === 'PT' ? 'Lisboa' : 'São Paulo',
          estimatedDays: isInternationalProduct ? 8 : 2,
        },
        specs: {
          ...initialProduct.specs,
          Marca: brand || initialProduct.specs?.Marca || 'Oficial',
          Modelo: model || initialProduct.specs?.Modelo || 'Padrão',
          Condição: condition === 'novo' ? 'Novo Lacrado' : 'Usado Conservado',
          Peso: `${parsedWeight} kg`,
          Dimensões: formattedDimensionsStr,
          'Peso com Embalagem': `${parsedWeight} kg`,
          Garantia: `${warrantyMonths} Meses com Nota Fiscal`,
          Armazém: warehouseHub,
          Origem: isInternationalProduct ? `Internacional (${getCountryName(originCountry)})` : `Nacional (${getCountryName(originCountry)})`,
        },
      };
    ...
    const created = onAddProduct({
      ...
      seller: {
        id: 'sel-user-my-store',
        name: selectedStoreName,
        reputationLevel: 'platinum',
        reputationScore: 5,
        salesCount: 150,
        isOfficialStore: true,
        location: { city: originCountry === 'GW' ? 'Bissau' : 'Lisboa', state: originCountry },
        goodService: true,
        onTimeDelivery: true,
      ...
      shipping: {
        freeShipping: true,
        arrivesTomorrow: !isInternationalProduct,
        shippingPrice: 0,
        fullFulfilled: true,
        isInternational: isInternationalProduct,
        originCountry,
        originCity: originCountry === 'GW' ? 'Bissau' : originCountry === 'PT' ? 'Lisboa' : 'São Paulo',
        targetCountries: effectiveTargetCountries,
        warehouseName: warehouseHub === 'HUB-GW-01' ? 'HUB Central Bissau' : warehouseHub === 'HUB-PT-02' ? 'HUB Lisboa Transit' : 'HUB São Paulo',
        estimatedDays: isInternationalProduct ? 8 : 2,
      },
      stock: parseInt(stock) || 10,
      salesCount: 1,
      description: description || `${title} com envio seguro via Nusali Logística.`,
      specs: {
        Marca: brand || 'Oficial',
        Modelo: model || 'Padrão',
        Condição: condition === 'novo' ? 'Novo Lacrado' : 'Usado Conservado',
    ...
    onOpenProductDetail(created.id);
  };
        ...
        {wizardStep === 4 && (
          <div className="space-y-6 text-xs font-medium">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-gray-800 font-bold mb-1">Preço de Venda (XOF/EUR) *</label>
                <input
                  type="number"
                  step="0.01"
                  value={price}
              ...
              <div>
                <label className="block text-gray-800 font-bold mb-1">Preço Riscado (Original)</label>
                <input
                  type="number"
                  step="0.01"
                  value={originalPrice}
              ...
              <div>
                <label className="block text-gray-800 font-bold mb-1">Estoque Total do Produto *</label>
                <input
                  type="number"
                  value={stock}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-gray-800 font-bold mb-1">Armazém Logístico Nusali (HUB Fulfillment) *</label>
                <select
                  value={warehouseHub}
                  onChange={(e) => setWarehouseHub(e.target.value as any)}
                  className="w-full p-2.5 border border-gray-300 rounded-xl bg-white font-bold"
                >
                  <option value="HUB-GW-01">HUB Central Bissau (Guiné-Bissau)</option>
                  <option value="HUB-PT-02">HUB Lisboa Transit (Portugal)</option>
                  <option value="HUB-BR-03">HUB São Paulo Guarulhos (Brasil)</option>
                </select>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold px-2.5 py-1 rounded-lg text-[11px] flex items-center gap-1 border border-blue-200"
                  >
                    <Sparkles className="w-3 h-3 text-blue-600" /> Galeria HD Exemplo
                  </button>
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3 py-1 rounded-lg text-[11px] flex items-center gap-1 shadow-xs"
                  >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold px-2.5 py-1 rounded-lg text-[11px] flex items-center gap-1 border border-emerald-200"
                  >
                    <Sparkles className="w-3 h-3 text-emerald-600" /> Usar Vídeo Demo
                  </button>
                  <button
                    type="button"
                    onClick={() => videoInputRef.current?.click()}
                    className="bg-slate-800 hover:bg-slate-900 text-white font-bold px-3 py-1 rounded-lg text-[11px] flex items-center gap-1 shadow-xs"
                  >
