...
export const ProductDetailView: React.FC = () => {
  ...
  const rawProduct = fetchedProduct || allProducts.find((p) => p.id === id) || allProducts[0];
  ...
  const installmentAmount = (effectiveTotalOrderPrice / instMax).toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  const formattedBasePrice = activeVariantPrice.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  const formattedEffectiveUnitPrice = effectiveUnitPrice.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  const formattedEffectiveTotal = effectiveTotalOrderPrice.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });

  const formattedOriginalPrice = activeVariantOriginalPrice?.toLocaleString('pt-BR', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
          ...
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-2">
            ...
            {selectedKit && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500 line-through">
                  R$ {(activeVariantPrice * selectedKit.quantity).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </span>
              </div>
            )}
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-gray-900">
                R$ {formattedEffectiveTotal}
              </span>
              {selectedKit && (
                <span className="text-xs text-gray-600 font-medium">
                  (R$ {formattedEffectiveUnitPrice} / cada)
                </span>
              )}
            </div>

            <p className="text-xs font-bold text-green-700">
              em {product.installmentsMax}x de R$ {installmentAmount}{' '}
            ...
            <div className="space-y-2.5 pt-2 border-t border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                >
                  ...
                  <span className="text-[11px] text-gray-500 font-medium">R$ {formattedBasePrice}</span>
                </button>
                    ...
                    >
                      ...
                      <div className="text-[11px]">
                        <span className="font-bold text-gray-900">
                          R$ {totalKit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                  ...
                  return (
                    ...
                    >
                      ...
                      {customVariantPrice !== undefined && customVariantPrice > 0 && (
                        ...
                        >
                          R$ {customVariantPrice.toLocaleString('pt-BR')}
                        </span>
                      )}
                    </button>
                  );
